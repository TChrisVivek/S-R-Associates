...
const DailyLogsTab = ({ projectId }) => {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500">Loading Daily Logs...</div>;
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-xl font-bold text-slate-900 mb-1">Project Daily Logs</h2>
                    <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
                        <Calendar size={14} />
                        <span>{logData.projectPhase}</span>
                        <span className="text-slate-300">•</span>
                        <span className="text-blue-600 font-bold uppercase">Last Updated: {logData.lastUpdated}</span>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <button onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-sm shadow-blue-500/20">
            ...
            <div className="relative pl-4">
                ...
                <div className="space-y-8">
                    {logData.logs.length === 0 ? (
                        <div className="pl-12 py-8 text-slate-500 font-medium italic">No daily logs recorded yet.</div>
                    ) : (
                        logData.logs.map((log) => (
                            <div key={log.id} className="relative pl-10">

                                {/* Timeline Dot Icon */}
                                <div className="absolute left-0 top-0 bg-blue-600 text-white w-10 h-10 rounded-full flex items-center justify-center border-4 border-slate-50 z-10 shadow-sm">
                                    <Calendar size={18} />
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
                                            {log.gallery && log.gallery.map((img, idx) => (
                                                <a key={idx} href={img} target="_blank" rel="noreferrer" className="w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden border border-slate-200 relative group cursor-pointer block">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm overflow-y-auto">
                    <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-2xl my-8 overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
                        <div className="px-8 py-6 flex flex-col border-b border-slate-100 bg-white relative">
                            <button onClick={() => setIsCreateModalOpen(false)} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 text-slate-400 transition-colors">
                                <X size={20} />
                            </button>
                            <h3 className="text-xl font-bold text-slate-900">Create Daily Log</h3>
                            <p className="text-slate-500 text-sm mt-1">Record today's progress, weather, and labor constraints</p>
                        </div>
                        ...
                        <form onSubmit={handleCreateLog} className="p-8 space-y-6 bg-slate-50/30">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="laborers" required placeholder="e.g. 42" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition-all shadow-sm text-sm" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-slate-700 mb-2">Progress Notes</label>
                                <textarea name="notes" rows="4" required placeholder="Detail the work completed today, any delays, safety checks, or material shortages..." className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all shadow-sm text-sm resize-none"></textarea>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-slate-700 mb-2">Site Gallery (Optional)</label>
                                <div className={`relative border-2 border-dashed ${galleryFiles.length > 0 ? 'border-blue-400 bg-blue-50/50' : 'border-slate-200 hover:border-blue-400 hover:bg-blue-50/50'} rounded-2xl p-6 flex flex-col items-center justify-center text-center transition-all group overflow-hidden bg-white`}>
                                    ...
                                    <div className={`w-10 h-10 ${galleryFiles.length > 0 ? 'bg-blue-100' : 'bg-slate-50 group-hover:bg-blue-100'} rounded-full flex items-center justify-center mb-3 transition-colors`}>
                                        <UploadCloud className={`${galleryFiles.length > 0 ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-600'} transition-colors`} size={20} />
                                    </div>
                                    <p className={`font-bold text-sm ${galleryFiles.length > 0 ? 'text-blue-700' : 'text-slate-700 group-hover:text-blue-700'} transition-colors`}>
                                ...
                                {galleryFiles.length > 0 && (
                                    <div className="mt-4 flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                                        {galleryFiles.map((file, idx) => (
                                            <div key={idx} className="relative w-16 h-16 rounded-lg border border-slate-200 overflow-hidden flex-shrink-0 group bg-slate-100">
                            ...
                            <div className="flex justify-end pt-4 gap-3">
                                <button type="button" onClick={() => { setIsCreateModalOpen(false); setGalleryFiles([]); }} className="px-6 py-3.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">
                                    Cancel
                                </button>
                                <button type="submit" disabled={actionLoading} className="px-8 py-3.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md shadow-blue-500/20 transition-all flex items-center gap-2 disabled:opacity-70">
