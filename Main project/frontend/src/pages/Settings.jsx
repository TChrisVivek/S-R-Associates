...
import {
    ...
    HardDrive, Cpu, Globe, Shield, Clock, Package, Plus
} from 'lucide-react';
...
const SettingsPage = () => {
        ...
        const fetchSettings = async () => {
            try {
                ...
                if (response.data) {
                    ...
                    if (response.data.labourRoles?.length) setLabourRoles(response.data.labourRoles);
                }
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await api.put('/settings', { companyInfo, notifications, labourRoles });
    ...
    const handleSaveLabourRoles = async () => {
        setIsSavingRoles(true);
        try {
            // Filter out any incomplete rows before saving
            const cleaned = labourRoles.filter(r => r.shortCode?.trim() && r.roleName?.trim());
            await api.put('/settings', { labourRoles: cleaned });
            setLabourRoles(cleaned);
            showToast('Labour role codes saved! Bot will use them on next startup.', 'success');
        } catch (err) {
            console.error('Failed to save labour roles:', err);
            showToast('Failed to save labour roles.', 'error');
        } finally {
    ...
    const sections = [
        ...
        { id: 'labour_roles', label: 'Labour Role Codes' },
                ...
                <main className="flex-1 overflow-y-auto bg-[#f6f7f9]">
                    ...
                    <div className="px-8 py-6 max-w-5xl mx-auto flex gap-8">
                        ...
                        <div className="flex-1 pb-24">
                            ...
                            {/* Labour Role Codes */}
                            {activeSection === 'labour_roles' && (
                                <div id="labour_roles" className="bg-white rounded-2xl border border-gray-100 p-6">
                                    <div className="flex items-start justify-between mb-1">
                                        <div>
                                            <h2 className="text-sm font-semibold text-gray-900">Labour Role Shortcodes</h2>
                                            <p className="text-xs text-gray-400 mt-0.5 max-w-md">
                                                Define what shortcodes like <code className="bg-gray-100 px-1 py-0.5 rounded text-[11px] font-mono">Fc</code> or <code className="bg-gray-100 px-1 py-0.5 rounded text-[11px] font-mono">MC</code> mean.
                                                The WhatsApp bot uses these to parse labour reports from site messages.
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => setLabourRoles(prev => [...prev, { shortCode: '', roleName: '' }])}
                                            id="add-labour-role-btn"
                                            className="flex items-center gap-1.5 text-xs font-semibold text-violet-700 bg-violet-50 hover:bg-violet-100 border border-violet-100 px-3 py-1.5 rounded-lg transition-all shrink-0"
                                        >
                                    ...
                                    {/* Info banner */}
                                    <div className="mt-4 mb-5 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 flex items-start gap-3">
                                        <Info size={14} className="text-indigo-500 shrink-0 mt-0.5" />
                                        <p className="text-[11px] text-indigo-700 leading-relaxed">
                                            After saving, restart the WhatsApp bot for new codes to take effect.
                                            Short codes are case-insensitive. Example: <strong>FC</strong> and <strong>fc</strong> are treated the same.
                                        </p>
                                    </div>
                                    ...
                                    <div className="rounded-xl border border-gray-100 overflow-hidden">
                                        <table className="w-full text-left text-sm">
                                            <thead className="bg-gray-50 border-b border-gray-100">
                                                <tr>
                                                    <th className="py-2.5 px-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider w-1/3">Shortcode</th>
                                                    <th className="py-2.5 px-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Role Name (shown in portal)</th>
                                            ...
                                            <tbody className="divide-y divide-gray-50">
                                                {labourRoles.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={3} className="py-8 text-center text-xs text-gray-400 italic">No role codes defined. Click "Add Code" to add one.</td>
                                                    </tr>
                                                ) : (
                                                    labourRoles.map((role, idx) => (
                                                        <tr key={idx} className="group hover:bg-gray-50/50 transition-colors">
                                                            <td className="py-2 px-4">
                                                                <input
                                                                    ...
                                                                    onChange={e => setLabourRoles(prev => prev.map((r, i) => i === idx ? { ...r, shortCode: e.target.value } : r))}
                                                                    placeholder="e.g. Fc"
                                                                    className="w-full font-mono text-xs bg-white border border-gray-200 rounded-lg px-3 py-2 focus:border-violet-400 focus:ring-1 focus:ring-violet-100 outline-none transition-all"
                                                                />
                                                            </td>
                                                            <td className="py-2 px-4">
                                                                <input
                                                                    ...
                                                                    type="text"
                                                                    value={role.roleName}
                                                                    onChange={e => setLabourRoles(prev => prev.map((r, i) => i === idx ? { ...r, roleName: e.target.value } : r))}
                                                                    placeholder="e.g. Female Helper"
                                                                    className="w-full text-xs bg-white border border-gray-200 rounded-lg px-3 py-2 focus:border-violet-400 focus:ring-1 focus:ring-violet-100 outline-none transition-all"
                                                                />
                                                            </td>
                                                            <td className="py-2 px-3">
                                                                <button
                                                                    id={`delete-role-${idx}`}
                                                                    onClick={() => setLabourRoles(prev => prev.filter((_, i) => i !== idx))}
                                    ...
                                    <div className="flex justify-end mt-4">
                                        <button
                                            id="save-labour-roles-btn"
                                            onClick={handleSaveLabourRoles}
                                            disabled={isSavingRoles}
                                            className="bg-[#1a1d2e] hover:bg-[#252840] text-white text-xs font-medium py-2 px-4 rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50"
                                        >
                                            {isSavingRoles ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                                            {isSavingRoles ? 'Saving...' : 'Save Role Codes'}
                                        </button>
                                    </div>
                                </div>
                            )}
