...
import {
    ...
    Plus, Image as ImageIcon, X, Loader2, UploadCloud, Shield
} from 'lucide-react';
                                ...
                                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-5 gap-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            {/* Clearance badge */}
                                            {log.clearanceType && (
                                                <div className="flex items-center gap-2 bg-teal-50 text-teal-700 px-3 py-1.5 rounded-lg text-xs font-medium border border-teal-100">
                                                    <Shield size={13} />
                                                    <span>{log.clearanceType} — {log.clearanceStatus || 'Cleared'}</span>
                                                </div>
                                            )}

                                            {/* Labour breakdown — individual role pills */}
                                            {log.labourBreakdown && log.labourBreakdown.length > 0 ? (
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    {log.labourBreakdown.map((r, i) => (
                                                        <div key={i} className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-2.5 py-1.5 rounded-lg text-xs font-medium border border-indigo-100">
                                                            <Users size={12} />
                                                            <span>{r.role}</span>
                                                            <span className="bg-indigo-200 text-indigo-800 rounded-md px-1.5 py-0.5 font-bold text-[10px] leading-none">×{r.count}</span>
                                                        </div>
                                                    ))}
                                                    {/* Total */}
                                                    <div className="flex items-center gap-1.5 bg-gray-100 text-gray-500 px-2.5 py-1.5 rounded-lg text-xs font-medium border border-gray-200">
                                                        <span>Total {log.laborers}</span>
                                                    </div>
                                                </div>
                                            ) : (
                                                /* Fallback — old entries without breakdown */
